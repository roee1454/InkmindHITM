import React from 'react'
import { artistColor } from '../artist-colors'
import type { ApiGoogleConnection } from '../types'

function GoogleIcon({ size = 13 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" className="shrink-0">
      <path
        fill="#4285F4"
        d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
      />
      <path
        fill="#34A853"
        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
      />
      <path
        fill="#FBBC05"
        d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.06H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.94l2.85-2.22.81-.63z"
      />
      <path
        fill="#EA4335"
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.06l3.66 2.84c.87-2.6 3.3-4.52 6.16-4.52z"
      />
    </svg>
  )
}

interface StaffItem {
  id: string
  name: string
  avatar?: string
}

interface ArtistLegendProps {
  staff: StaffItem[]
  selectedArtist: string
  onSelectedArtistChange: (artistId: string) => void
  googleConnections?: ApiGoogleConnection[]
}

export const ArtistLegend: React.FC<ArtistLegendProps> = ({
  staff,
  selectedArtist,
  onSelectedArtistChange,
  googleConnections = [],
}) => {
  return (
    <div className="flex flex-wrap items-center justify-start gap-2 font-assistant" dir="rtl">
      {staff.map((artist) => {
        const active = selectedArtist === artist.id
        const googleConn = googleConnections.find((c) => c.staffId === artist.id)
        const isGoogleConnected = googleConn?.status === 'connected'
        const avatarUrl = googleConn?.googleAccountPicture || artist.avatar

        return (
          <button
            key={artist.id}
            type="button"
            onClick={() => onSelectedArtistChange(active ? 'all' : artist.id)}
            className={`flex items-center gap-2 border px-3 py-1.5 text-xs font-semibold transition-colors cursor-pointer rounded-xl ${
              active
                ? 'border-primary bg-primary/10 text-foreground font-bold shadow-sm'
                : 'border-border bg-card text-muted-foreground hover:text-foreground'
            }`}
            aria-pressed={active}
          >
            {avatarUrl ? (
              <img
                src={avatarUrl}
                alt=""
                className="h-4 w-4 rounded-full object-cover shrink-0 border border-border"
                referrerPolicy="no-referrer"
              />
            ) : isGoogleConnected ? (
              <span className="flex items-center shrink-0" title="מחובר ל-Google Calendar">
                <GoogleIcon size={13} />
              </span>
            ) : (
              <span className={`h-2 w-2 rounded-full ${artistColor(artist.id).dot}`} />
            )}
            <span>{artist.name}</span>
            {isGoogleConnected && (
              <span className="flex items-center text-emerald-500 shrink-0" title="סנכרון Google Calendar פעיל">
                <GoogleIcon size={12} />
              </span>
            )}
          </button>
        )
      })}
    </div>
  )
}

export default ArtistLegend
